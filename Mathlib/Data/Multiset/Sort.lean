...
/-- `sortBy s` constructs a sorted list from the multiset `s`.
  (Uses merge sort algorithm.) -/
def sortBy (s : Multiset α) : List α :=
...
@[simp]
theorem coe_sortBy (l : List α) : sortBy r l = mergeSort l (r · ·) :=
  rfl

@[simp]
theorem sortBy_sorted (s : Multiset α) : Sorted r (sortBy r s) :=
  Quot.inductionOn s (sorted_mergeSort' _)

@[simp]
theorem sortBy_eq (s : Multiset α) : ↑(sortBy r s) = s :=
  Quot.inductionOn s fun _ => Quot.sound <| mergeSort_perm _ _

@[simp]
theorem mem_sortBy {s : Multiset α} {a : α} : a ∈ sortBy r s ↔ a ∈ s := by rw [← mem_coe, sortBy_eq]

@[simp]
theorem length_sortBy {s : Multiset α} : (sortBy r s).length = card s :=
  Quot.inductionOn s <| length_mergeSort

@[simp]
theorem sortBy_zero : sortBy r 0 = [] :=
  List.mergeSort_nil
...
theorem map_sortBy (f : α → β) (s : Multiset α)
    (hs : ∀ a ∈ s, ∀ b ∈ s, r a b ↔ r' (f a) (f b)) :
    (s.sortBy r).map f = (s.map f).sortBy r' := by
  revert s
  exact Quot.ind fun l h => map_mergeSort (l := l) (by simpa using h)

theorem sortBy_cons (a : α) (s : Multiset α) :
    (∀ b ∈ s, r a b) → sortBy r (a ::ₘ s) = a :: sortBy r s := by
