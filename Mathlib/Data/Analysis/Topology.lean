...
structure Ctop (α σ : Type*) where
  /-- Interpret an index `s : σ` as a subset of `α`; the sets `f s` form the basis. -/
  f : σ → Set α
  /-- An index of a basic set containing `x`, for each point `x`. -/
  top : α → σ
  /-- `x` lies in the basic set `top x`. -/
  top_mem : ∀ x : α, x ∈ f (top x)
  /-- For indices `a b` and a point `x ∈ f a ∩ f b`, an index of a basic set containing `x` and
  contained in `f a ∩ f b`. -/
  inter : ∀ (a b) (x : α), x ∈ f a ∩ f b → σ
  /-- `x` lies in the basic set `inter a b x h`. -/
  inter_mem : ∀ a b x h, x ∈ f (inter a b x h)
  /-- The basic set `inter a b x h` is contained in `f a ∩ f b`. -/
...
structure Ctop.Realizer (α) [T : TopologicalSpace α] where
  /-- The type of indices of the realizing basis. -/
  σ : Type*
  /-- The `Ctop` on `σ` generating the topology `T`. -/
  F : Ctop α σ
  /-- The topology generated by `F` is `T`. -/
  eq : F.toTopsp = T
...
structure LocallyFinite.Realizer [TopologicalSpace α] (F : Ctop.Realizer α) (f : β → Set α) where
  /-- For each point `a`, an index of a basic open set of `F` containing `a`. -/
  bas : ∀ a, { s // a ∈ F.F s }
  /-- For each point `x`, a `Fintype` structure on the set of `i` such that `f i` meets the basic
  neighbourhood `bas x`, witnessing that only finitely many do. -/
