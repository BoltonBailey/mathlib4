...
structure CFilter (α σ : Type*) [PartialOrder α] where
  /-- Interpret an index `s : σ` as an element of `α`; the elements `f s` form the filter base. -/
  f : σ → α
  /-- A distinguished index, the top element of the base. -/
  pt : σ
  /-- For indices `a b`, an index of a lower bound of `f a` and `f b` in the base. -/
  inf : σ → σ → σ
  /-- `inf a b` lies below `a`. -/
  inf_le_left : ∀ a b : σ, f (inf a b) ≤ f a
  /-- `inf a b` lies below `b`. -/
...
structure Filter.Realizer (f : Filter α) where
  /-- The type of indices of the realizing filter base. -/
  σ : Type*
  /-- The `CFilter` on `σ` whose generated filter is `f`. -/
  F : CFilter (Set α) σ
  /-- The filter generated by `F` is `f`. -/
  eq : F.toFilter = f
