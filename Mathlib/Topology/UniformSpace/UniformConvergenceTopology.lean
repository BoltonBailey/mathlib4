...
/-- `phi α β ((u, v), x) = (u x, v x)`: evaluate a pair of functions in `α →ᵤ β` at a point. Pushing
the filter `𝓐 ×ˢ ⊤` forward along it gives the lower adjoint of `UniformFun.filter` used in
`UniformFun.gc`. -/
