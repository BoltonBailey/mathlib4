...
/-- Given a numeral `n'`, produce numerals `a' = fib n'` and `b' = fib (n' + 1)` with a proof of
`IsFibAux n' a' b'`. The recursion follows the binary expansion of `n'`, using the doubling
formulas `isFibAux_two_mul` and `isFibAux_two_mul_add_one`, so it takes logarithmically many
steps. -/
