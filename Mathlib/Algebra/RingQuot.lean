...
structure RingQuot (r : R → R → Prop) where
  /-- The underlying element of the quotient of `R` by the ring congruence generated by `r`. -/
...
/-- Lift a ring hom `f : R →+* T` that respects `r` to a ring hom `RingQuot r →+* T`. This is the
underlying construction of `RingQuot.lift`, which packages it as an equivalence. -/
...
/-- Lift an algebra hom `f : A →ₐ[S] B` that respects `s` to an algebra hom `RingQuot s →ₐ[S] B`.
This is the underlying construction of `RingQuot.liftAlgHom`, which packages it as an
equivalence. -/
