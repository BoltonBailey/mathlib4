...
structure RingQuot (r : R → R → Prop) where
  /-- The underlying quotient of `R` by the congruence generated by `r`. -/
...
/-- The ring hom `RingQuot r →+* T` induced by a ring hom `f : R →+* T` that respects `r`. This is
the forward direction of the equivalence `RingQuot.lift`. -/
...
/-- The algebra hom `RingQuot s →ₐ[S] B` induced by an algebra hom `f : A →ₐ[S] B` that respects
`s`. This is the forward direction of the equivalence `RingQuot.liftAlgHom`. -/
