...
class Decomposition where
  /-- The unbundled decomposition map, sending an element of `M` to its family of homogeneous
  components. -/
  decompose' : M → ⨁ i, ℳ i
  /-- `decompose'` is a left inverse of the recomposition map. -/
  left_inv : Function.LeftInverse (DirectSum.coeAddMonoidHom ℳ) decompose'
  /-- `decompose'` is a right inverse of the recomposition map. -/
