...
/-- A label naming a continuation `α → m β`. `callCC` hands one of these to its argument, and
`MonadCont.goto` invokes it. -/
structure MonadCont.Label (α : Type w) (m : Type u → Type v) (β : Type u) where
  /-- The continuation itself. Prefer `MonadCont.goto` to calling this directly. -/
  apply : α → m β

/-- Jump to the label `f` with the value `x`, i.e. invoke the continuation it names. -/
abbrev MonadCont.goto {α β} {m : Type u → Type v} (f : MonadCont.Label α m β) (x : α) :=
  f.apply x

/-- A monad with a call-with-current-continuation operator `callCC`. -/
class MonadCont (m : Type u → Type v) where
  /-- `callCC f` runs `f` with a label for the current continuation. Jumping to that label from
  inside `f` with a value `x` makes `x` the result of the whole `callCC f`. -/
  callCC : ∀ {α β}, (MonadCont.Label α m β → m α) → m α

open MonadCont

/-- The laws expected of `callCC`: it commutes with binding a command that precedes it, jumping to
the label immediately returns the jumped value, and a `callCC` whose label is never used is just
its body. -/
...
/-- The continuation monad transformer. A value of `ContT r m α` is a function `(α → m r) → m r`: it
delivers its result of type `α` to a continuation, which computes the final answer in `m r`. -/
def ContT (r : Type u) (m : Type u → Type v) (α : Type w) :=
  (α → m r) → m r

/-- The continuation monad `Cont r α`, that is, `ContT r` over the identity monad. -/
...
/-- Apply `f : m r → m r` to the final answer of a continuation computation. -/
...
/-- Change the result type of a continuation computation from `α` to `β` by transforming the
continuation it receives: `withContT f x` handed `g : β → m r` runs `x` with `f g`. -/
...
/-- Turn a label accepting `Except ε α` in `m` into a label accepting `α` in `ExceptT ε m`, by
jumping with `Except.ok a`. -/
...
/-- `callCC` for `ExceptT ε m`: unwrap to `m`, use `callCC` there, and convert the label with
`ExceptT.mkLabel`. -/
...
/-- Turn a label accepting `Option α` in `m` into a label accepting `α` in `OptionT m`, by jumping
with `some a`. -/
...
/-- `callCC` for `OptionT m`: unwrap to `m`, use `callCC` there, and convert the label with
`OptionT.mkLabel`. -/
...
/-- Turn a label accepting `α × ω` in `m` into a label accepting `α` in `WriterT ω m`, by jumping
with the empty output `(a, ∅)`. -/
def WriterT.mkLabel {α β ω} [EmptyCollection ω] : Label (α × ω) m β → Label α (WriterT ω m) β
  | ⟨f⟩ => ⟨fun a => monadLift <| f (a, ∅)⟩

/-- Variant of `WriterT.mkLabel` for a monoid `ω`, jumping with the trivial output `(a, 1)`. -/
...
/-- `callCC` for `WriterT ω m` with `ω` an `EmptyCollection`, via `WriterT.mkLabel`. -/
...
/-- `callCC` for `WriterT ω m` with `ω` a monoid, via `WriterT.mkLabel'`. -/
...
/-- Turn a label accepting `α × σ` and returning `β × σ` in `m` into a label accepting `α` in
`StateT σ m`, by jumping with the state current at the time of the jump. -/
...
/-- `callCC` for `StateT σ m`: for each initial state, use `callCC` in `m` and convert the label
with `StateT.mkLabel`. -/
...
/-- Lift a label in `m` to a label in `ReaderT ρ m`; the environment plays no role. -/
...
/-- `callCC` for `ReaderT ε m`: under the current environment, use `callCC` in `m` and convert the
label with `ReaderT.mkLabel`. -/
