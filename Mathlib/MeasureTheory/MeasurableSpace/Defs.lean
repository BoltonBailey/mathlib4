...
section CountableCocountable

/--
The `MeasurableSpace` consisting of exactly those sets which are countable or cocountable.
-/
def CountableCocountable : MeasurableSpace α :=
  generateFrom ({{a} | a : α})


lemma CountableCocountable.mem_iff {s : Set α} :
    MeasurableSet[CountableCocountable] s ↔ s.Countable ∨ sᶜ.Countable := by
  simp [CountableCocountable, generateFrom, GenerateMeasurable, setOf_exists]
  sorry

/--
The `MeasurableSpace` of countable and cocountable sets
is that generated by all singleton sets of `α`.
-/
lemma CountableCocountable.eq_generateFrom_singleton :
    CountableCocountable = generateFrom ({{a} | a : α}) := rfl

/--
A measurable space is `MeasurableSingletonClass`
if and only if it is finer than `CountableCocountable`.
-/
lemma MeasurableSingletonClass_iff_le_CountableCocountable [MeasurableSpace α] :
    MeasurableSingletonClass α ↔ CountableCocountable ≤ ‹MeasurableSpace α› := by
  sorry

end CountableCocountable
